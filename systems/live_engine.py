...
def _run_iteration(settings, runtime_states, *, dry: bool, verbose: int) -> None:
    for name, ledger_cfg in settings.get("ledger_settings", {}).items():
        ...
        jackpot_cfg = settings.get("jackpot", {})
        j_state = state.get("jackpot_state")
        stats = compute_strategy_report(ledger_obj, {"_": price})
        for strat in sorted(stats.keys()):
            ...
            addlog(
                f"[REPORT][{strat}] buys={s['buys']} sells={s['sells']} realized_pnl=${realized_pnl:.2f} realized_roi={(realized_roi*100):.2f}% avg_trade_roi={(avg_trade_roi*100):.2f}% open_notes_value=${open_value:.2f} window_total_at_liq=${total_at_liq:.2f}",
                verbose_int=1,
                verbose_state=verbose,
            )
        if "jackpot" in stats and j_state is not None:
            addlog(
        ...
        sum_open = sum(s["open_value_now"] for s in stats.values())
