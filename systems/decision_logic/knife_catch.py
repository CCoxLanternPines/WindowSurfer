...
def should_buy_knife(
    ...
    if cooldowns["knife_catch"] <= 0:
        if tunnel_pos is not None and abs(tunnel_pos) < 0.05:
...
def should_sell_notes(notes: list, candle: dict, settings: dict, verbose: int = 0) -> list:
    """Return all knife notes if all are positive and one meets margin threshold."""
    ...
    return []  # All positive, but no margin met
